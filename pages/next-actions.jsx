...
export default function NextActions(){
  ...
  const createTasks = async (c)=>{
    await createOutlookTask({ title:`Recall con ${c.name}`, due: new Date().toISOString() });
    await createZohoTask({ title:`Follow-up ${c.name}`, due: new Date().toISOString() });
    alert("Task creati (stub) ✅");
  }
      ...
      <div className="grid md:grid-cols-3 gap-4">
        {ranked.map((c,i)=>(
          <div key={i} className="card">
            ...
            <button className="btn mt-3" onClick={()=>createTasks(c)}>Crea task (stub)</button>
          </div>
        ))}
      </div>
    </Layout>
